classdef CSV < mlunit.test_case
  ...
  methods
    ...
    function test_obj = CSV(varargin)
      test_obj = test_obj@mlunit.test_case(varargin{:});
    end
    
    function self = set_up( self )
      % Load baseline data
      BASE = load(fullfile('baselines','baseline_IO_CSV.mat'),'-MAT');
      self.baseObj.CDS = BASE.CDS;
      % Run CANARY
      FILE = dir(fullfile('test_io_CSV.edsd'));
      if isempty(FILE),
        canary('test_io_CSV.edsx');
      end
      % Load output data
      TEST = load(fullfile('test_io_CSV.edsd'),'-MAT');
