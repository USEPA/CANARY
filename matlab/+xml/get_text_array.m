function [ result ] = get_text_array( xmlObj, name )
  ...
  xChd = xmlObj.getElementsByTagName(name);
  if isempty(xChd); return; end;
  for i = 0:(xChd.getLength() - 1)
    child = xChd.item(i);
    result(end+1) =  child.getTextContent() ;
    child = [];
  end
  xChd = [];
  xmlObj = [];
