classdef test_DataSource < mlunit.test_case
  ...
  methods
    ...
    function testds_obj = test_DataSource(varargin)
      testds_obj = testds_obj@mlunit.test_case(varargin{:});
      testds_obj.testObj = cws.DataSource('UNITTEST');
    end
    
    % Public Unit Tests
    function self = test_01_create( self )
      mlunit.assert(isa(self.testObj,'cws.DataSource'));
      mlunit.assert(strcmp('UNITTEST',self.testObj.conn_id));
    end
